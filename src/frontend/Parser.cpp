...
{
  static bool
  replaceSubstr( std::string& str, const std::string& from, const std::string& to )
  {
  ...
  Parser::processArgs( int mpiRank, int argc, char** argv )
  {
    ...
    for ( int i = 1; i < argc; i++ )
    {
      ...
      if ( opt.find( "--input=" ) != string::npos ) {
        options.inFileName = opt.erase( 0, string( "--input=" ).length( ) );
      }
      /*  output file */
      else
      if ( opt.compare( string( "-o" ) ) == 0 ||
      ...
      if ( opt.find( "--verbose=" ) != string::npos ) {
      ...
      if ( opt.find( "--no-csv" ) != string::npos ) {
        options.createRatingCSV = false;
      }
      /* print top X activities */
      else
      if ( opt.compare( string( "-t" ) ) == 0 ||
      ...
      if ( opt.find( "--no-errors=" ) != string::npos ) {
        options.noErrors = true;
      }
      /* do not apply rules for non-blocking MPI */
      else
      if ( opt.find( "--ignore-impi" ) != string::npos ) {
      ...
      if ( opt.find( "--ignore-offloading" ) != string::npos ) {
      ...
      if ( opt.find( "--ignore-cuda-events" ) != string::npos ) {
      ...
      if ( opt.find( "--link-kernels=" ) != string::npos ) {
        options.linkKernels =
      ...
      if ( opt.find( "--nullstream=" ) != string::npos ) {
        options.nullStream =
      ...
      if ( opt.find( "--interval-analysis=" ) != string::npos ) {
