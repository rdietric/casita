...
int
OTF1TraceReader::getProcessingPhase( )
{
  return processingPhase;
}

uint32_t
OTF1TraceReader::getMPIRank( )
{
  return mpiRank;
}

uint32_t
OTF1TraceReader::getMPIProcessId( )
{
  return mpiProcessId;
}

void
OTF1TraceReader::setMPIProcessId( uint32_t processId )
{
  mpiProcessId = processId;
}

OTF1TraceReader::TokenTokenMap&
OTF1TraceReader::getProcessRankMap( )
{
  return processRankMap;
}

OTF1TraceReader::TokenTokenMap&
OTF1TraceReader::getProcessFamilyMap( )
{
  return processFamilyMap;
}

void
OTF1TraceReader::open( const std::string otfFilename, uint32_t maxFiles )
{
...
void
OTF1TraceReader::setCommEventHandlers( OTF_HandlerArray* handlers )
{
...
void
OTF1TraceReader::setEventHandlers( OTF_HandlerArray* handlers )
{
 ...
 */
void
OTF1TraceReader::readCommunication( )
{
  OTF_HandlerArray* handlers = OTF_HandlerArray_open( );
...
void
OTF1TraceReader::readDefinitions( )
{
  OTF_HandlerArray* handlers = OTF_HandlerArray_open( );
  ...
  /* disable reading for all processes which are not in the process
   * map */
...
OTF1KeyValueList&
OTF1TraceReader::getKVList( )
{
  return kvList;
}

OTF1TraceReader::NameTokenMap&
OTF1TraceReader::getNameKeysMap( )
{
  return nameKeysMap;
}

OTF1TraceReader::TokenNameMap&
OTF1TraceReader::getKeyNameMap( )
{
  return kNameMap;
}

OTF1TraceReader::TokenNameMap&
OTF1TraceReader::getFuncNameMap( )
{
  return fNameMap;
}

std::string
OTF1TraceReader::getKeyName( uint32_t id )
{
...
OTF1TraceReader::TokenNameMap&
OTF1TraceReader::getProcNameMap( )
{
  return pNameMap;
}

OTF1TraceReader::ProcessGroupMap&
OTF1TraceReader::getProcGoupMap( )
{
  return processGroupMap;
}

OTF1TraceReader::AttrListMap&
OTF1TraceReader::getAttrListMap( )
{
  return attrListMap;
}

OTF1TraceReader::ProcessFuncStack&
OTF1TraceReader::getFuncStack( )
{
  return funcStack;
}
...
std::vector< uint32_t >
OTF1TraceReader::getKeys( const std::string keyName )
{
...
int32_t
OTF1TraceReader::getFirstKey( const std::string keyName )
{
  std::pair< NameTokenMap::iterator, NameTokenMap::iterator > range;
  range = nameKeysMap.equal_range( keyName );
...
uint64_t
OTF1TraceReader::getTimerResolution( )
{
  return ticksPerSecond;
}

void
OTF1TraceReader::setTimerResolution( uint64_t ticksPerSecond )
{
  this->ticksPerSecond = ticksPerSecond;
}

bool
OTF1TraceReader::isChildOf( uint32_t child, uint32_t parent )
{
                                       ...
                                       OTF_KeyValueList_struct* list )
{
  ...
  if ( phase == 2 )
  {
    if ( tr->handleProcessMPIMapping )
    {
      if ( parent == 0 )
      {
        tr->handleProcessMPIMapping( tr, processId,
                                     tr->getProcessRankMap( )[processId] );
      }
      else
      {
        tr->handleProcessMPIMapping( tr, processId,
                                     tr->getProcessRankMap( )[parent] );
      }
    }

    /* skip all processes but the mapping MPI master process and its
     * children */
    if ( processId != tr->getMPIProcessId( ) &&
         ( !tr->isChildOf( processId, tr->getMPIProcessId( ) ) ) )
    {
      return OTF_RETURN_OK;
    }
...
std::list< OTF1TraceReader::OTF1CommEvent >&
OTF1TraceReader::getCommEventList( uint32_t processId )
{
  return commEventListMap[processId];
}

OTF1TraceReader::OTF1CommEvent
OTF1TraceReader::getCurrentCommEvent( uint32_t processId )
{
...
uint64_t
OTF1TraceReader::getCurrentCommEventTime( uint32_t processId )
{
  if ( commEventListMap[processId].size( ) > 0 )
  {
...
std::vector< OTF1TraceReader::OTF1CollBeginOp >&
OTF1TraceReader::getCollBeginList( uint32_t processId )
{
  return collBeginListMap[processId];
}

std::vector< OTF1TraceReader::OTF1CollEndOp >&
OTF1TraceReader::getCollEndList( uint32_t processId )
{
  return collEndListMap[processId];
}

std::vector< OTF1TraceReader::OTF1RMAEnd >&
OTF1TraceReader::getRmaEndList( uint32_t processId )
{
  return rmaEndListMap[processId];
}

std::vector< OTF1TraceReader::OTF1RMAGet >&
OTF1TraceReader::getRmaGetList( uint32_t processId )
{
  return rmaGetListMap[processId];
}

std::vector< OTF1TraceReader::OTF1RMAPut >&
OTF1TraceReader::getRmaPutList( uint32_t processId )
{
  return rmaPutListMap[processId];
}

std::vector< OTF1TraceReader::OTF1SendMsg >&
OTF1TraceReader::getSendMsgList( uint32_t processId )
{
  return sendMsgListMap[processId];
}

std::vector< OTF1TraceReader::OTF1RecvMsg >&
OTF1TraceReader::getRecvMsgList( uint32_t processId )
{
